...
Hc_ac_m = g2*xc.dag()*(Sac)  # b -> a, a-dag & sigma minus, time-dependent term
def Hc_ac_m_coeff(t, args):
    ab_detun = np.exp(1j*((Delta_1)*t))
    return ab_detun

Hc_ab_m = g1*xc.dag()*(Sab)  # c -> a, a-dag & sigma minus, time-dependent term
def Hc_ab_m_coeff(t, args):
    ac_detun = np.exp(1j*((Delta_2)*t))
    return ac_detun

Hc_ac_p = g2*xc*(Sac.dag())  # a -> b, a & sigma plus, time-dependent term
def Hc_ac_p_coeff(t, args):
    ab_detun = np.exp(-1j*((Delta_1)*t))
    return ab_detun

Hc_ab_p = g1*xc*(Sab.dag())  # a -> c, a & sigma plus, time-dependent term
